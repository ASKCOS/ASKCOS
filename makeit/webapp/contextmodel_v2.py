...
'''
v2 is meant for FPs without features
'''
...
class NNConditionPredictor():
    ...
    def load_db_model(self, db_client, model_dir, model_name='fp256noFtr_NN10_BT.pickle'):
        ...
        if self.model_name == 'fpNN-10_BT_256Full.pickle':
            self.useFeatures = True
        elif self.model_name == 'fp256noFtr_NN10_BT.pickle':
            self.useFeatures = False
        else:
            print('Wrong model_name?! Use fp256noFtr_NN10_BT.pickle instead.')

        # Load the nearest neighbor model
        with open(os.path.join(model_dir, model_name), 'rb') as infile:
    ...
    def step_n_conditions(self, n, rxn):
        ...
        try:
            rxn_fp = create_rxn_Morgan2FP(rxn, fpsize=256, useFeatures=self.useFeatures)
            ...
            # with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'NN_info.txt'), 'w') as outfile:
            #     outfile.write('No.\tRxn ID\tDist')
            #     for i, dist in enumerate(dists[0]):
            #         outfile.write('{}\t{}\t{}'.format(i, self.rxn_ids[ids[0][i]], dist))
    ...
    def path_condition(self, n, path):
        ...
        rxn_fps = []
        for rxn in path:
            rxn_fps.append(create_rxn_Morgan2FP(rxn, fpsize=256, useFeatures=self.useFeatures))
