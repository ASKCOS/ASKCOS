...
def coordinator(workers_done, coordinator_done, apply_queue, results_queue, done, model, context, products, intended_product, plausible, quit_if_unplausible):
	...
	while True:
		...
		try:
			...
			# Save score in shared dictionary
			if products.has_key(product_smiles):
				products[product_smiles] = max(products[product_smiles], score)
			else:
				products[product_smiles] = score 
			...
			if product_smiles == intended_product:
				intended_found = True 
				intended_score = products[product_smiles]
...
if __name__ == '__main__':
	...
	predictor.set_context(T = '20', reagents = '[Na+].[O-]', solvent = 'water')
	...
	predictor.run_in_foreground(reactants = 'CCCCCCCO.CC[Br]', intended_product = '', quit_if_unplausible = False)
	outcomes = predictor.return_top(n = 3)
	print(outcomes)
