...
if __name__ == '__main__':
	...
	for doc in tqdm(INSTANCE_DB.find({}, ['RXD_RGTXRN'], no_cursor_timeout = True)):
	...
	plt.title('Most popular reagents in Reaxys ({} instances)'.format(N_rxdids))
	plt.grid(True)
	N_top = 20
	...
	fig.savefig(os.path.join(out_folder, 'reagents.png'))
	plt.show()

	with open(os.path.join(out_folder, 'reagents.csv'), 'w') as fid:
