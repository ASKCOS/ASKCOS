...
db = client['reaxys_v2']
TRANSFORM_DB = db['transforms_forward_v2']
...
USE_STEREOCHEMISTRY = False # for now
...
def convert_atom_to_wildcard(atom):
    ...
    if SUPER_GENERAL_TEMPLATES:
        if ':' in atom.GetSmarts():
            ...
            return '[#{}{}'.format(atom.GetAtomicNum(), label.group())
...
def process_an_example_doc(example_doc, test=False):
    ...
    try:
        ...
        for rxd_id in rxd_id_list:
            ...
            if extra_reactant_fragment:
                rxd = INSTANCE_DB.find_one({'_id': rxd_id}, ['RXD_RGTXRN'])
                if v: print(' Instance {}'.format(rxd['_id']))
                reagents = []
                num_matches = []
                xrns = []
                for xrn in rxd['RXD_RGTXRN']:
                    smi = CHEMICAL_DB.find_one({'_id': xrn}, ['SMILES'])['SMILES']
                ...
                if sum(num_matches) == 0: 
                    if v: print('    none of the reagents have {}, skpping this RXD'.format(extra_reactant_fragment[1:-1]))
                    continue
            ...
            if correct or DO_NOT_TEST:  
                ...
                if test:
                    ...
                    if v: print('Added record for template {}'.format(rxn_canonical))
            else:
...
def main(N = 15, skip = 0, skip_id = 0):
    ...
    res = Parallel(n_jobs=8, verbose=5, pre_dispatch=500)(delayed(process_an_example_doc)(data) for data in generator)
