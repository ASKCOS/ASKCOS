...
v2 changes how context is incorporated into the prediction. Instead of forming a single enhancement
factor, it adds the reagent fingerprint effect to the hidden representation vector.
...
def build(F_atom = 1, F_bond = 1, N_e = 5, N_h1 = 100, N_h2 = 50, N_h3 = 0, N_c = 100, inner_act = 'tanh',
	...
	# Add enhancement
	net_sum_w_reagents = merge([net_sum_h, reagents_h_rpt], mode = 'sum', name = "add reaction with reagents")

	# Converge to G0, C[not real], E, S, A, B, V, and K
	feature_to_params = Dense(8, activation = 'linear', W_regularizer = l2(l2v))
	params = TimeDistributed(feature_to_params, name = "features to K,G0,C,E,S,A,B,V")(net_sum_w_reagents)
