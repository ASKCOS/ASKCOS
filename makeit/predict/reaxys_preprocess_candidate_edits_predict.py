...
def get_candidates(candidate_collection, n = 2, seed = None, outfile = '.', shuffle = False, 
	...
	for j, reaction in generator:
		...
		if padUpTo: zipsort = zipsort[:padUpTo]
		...
		if i % n == 0:
			...
			with open(os.path.join(FROOT, '{}-{}_reactant_dicts.pickle'.format(i-n, i-1)), 'wb') as outfile:
				pickle.dump(np.array(reactant_dicts), outfile, pickle.HIGHEST_PROTOCOL)
			with open(os.path.join(FROOT, '{}-{}_candidate_edits_compact.pickle'.format(i-n, i-1)), 'wb') as outfile:
