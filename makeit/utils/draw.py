...
def MolToImage(mol, max_size=(1000, 1000), kekulize=True, options=None,
    ...
    if mol == '->':
        ...
        p3 = (subImgSize[0]-20, subImgSize[1]//2-10)
        p4 = (subImgSize[0]-20, subImgSize[1]//2+10)
        canvas.addCanvasLine((10, b//2-7), (a-17, b//2-7), lineWidth=2, color=(0, 0, 0))
        canvas.addCanvasLine((10, b//2+7), (a-17, b//2+7), lineWidth=2, color=(0, 0, 0))
        canvas.addCanvasLine((a-24, b//2-14), (a-10, b//2), lineWidth=2, color=(0, 0, 0))
        canvas.addCanvasLine((a-24, b//2+14), (a-10, b//2), lineWidth=2, color=(0, 0, 0))
        if hasattr(canvas, 'flush'):
            canvas.flush()
        else:
...
def ReactionToImage(rxn, dummyAtoms=False, options=None, **kwargs):
    ...
    imgs = [TrimImgByWhite(MolToImage(
        mol, kekulize=False, options=options), padding=15) for mol in mols]
...
def ReactionStringToImage(rxn_string, strip=True, update=True, options=None, 
    ...
    if strip:
        mols = [Chem.MolFromInchi(Chem.MolToInchi(mol))
...
def TransformStringToImage(transform, retro=True, **kwargs):
    ...
    return ReactionToImage(rxn, dummyAtoms=True, options=options, 
        retro=retro, **kwargs)
