...
def summarize_reaction_outcome_use_isotopes(mols, outcome):
	...
	conserved_maps = [a.GetIsotope() for a in outcome.GetAtoms() if a.GetIsotope()]
	changes = 0
	
	for atom_prev in mols.GetAtoms():
		atomMapNumber = atom_prev.GetIsotope()
		atom_new = [a for a in outcome.GetAtoms() if a.GetIsotope() and a.GetIsotope() == atomMapNumber]
...
def is_desired_product(outcome, desired):
	if isinstance(outcome, str):
		...
		smiles1 = Chem.MolToSmiles(Chem.MolFromSmiles(outcome))
	else:
		smiles1 = Chem.MolToSmiles(outcome)
		
	if isinstance(desired, str):
		#Make sure cannonical smiles
		smiles2 = Chem.MolToSmiles(Chem.MolFromSmiles(desired))
	else:
		smiles2 = Chem.MolToSmiles(desired)
