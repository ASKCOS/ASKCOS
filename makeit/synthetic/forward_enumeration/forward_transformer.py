...
class ForwardTransformer(TemplateTransformer, ForwardEnumerator):
    ...
    def get_prioritizers(self, template_prioritizer):
        if not template_prioritizer:
            MyLogger.print_and_log(
                'Cannot run the synthetic transformer without a template prioritization method. Exiting...', forward_transformer_loc, level=3)
        if template_prioritizer in self.template_prioritizers:
            ...
            if template_prioritizer == gc.popularity:
                ...
                MyLogger.print_and_log(
                    'Prioritization method not recognized. Using literature popularity prioritization.', forward_transformer_loc, level=1)
    ...
    def load(self, lowe=False, refs=False, efgs=False, rxns=True):
        ...
        for document in self.TEMPLATE_DB.find(filter_dict, to_retrieve):
            ...
            if rxns:  # Load into RDKit
                try:
                    if lowe:
                        reaction_smarts_synth = '(' + reaction_smarts.split(
                    ...
                    MyLogger.print_and_log('Couldnt load forward: {}: {}'.format(
                        reaction_smarts_synth, e), forward_transformer_loc, level=1)
                    template['rxn_f'] = None
    ...
    def dump_to_file(self, file_name):
        ...
        MyLogger.print_and_log('Wrote templates to {}'.format(os.path.join(
            gc.retro_template_data, file_name)), forward_transformer_loc)
    ...
    def load_from_file(self, file_name, rxns=True):
        ...
        configuration
        '''
        MyLogger.print_and_log('Loading templates from {}'.format(
            file_name), forward_transformer_loc)
        if os.path.isfile(os.path.join(gc.synth_template_data, file_name)):
            ...
            MyLogger.print_and_log(
                "No file to read data from, using online database instead.", forward_transformer_loc, level=1)
            self.load()
        self.num_templates = len(self.templates)
    ...
    def top_templates(self, target):
        ...
        for template in prioritized_templates:
            ...
            # do not break on count<mincount: assumes sorted by popularity, not
            # necessarily true
