...
class TreeEvaluator():
    ...
    def __init__(self, celery = False, rank_inclusion = 10, prob_inclusion = 0.2, max_contexts = 10, single_solv = True,
        ...
        if not self.celery:
            self.context_recommender = model_loader.load_Context_Recommender(context_recommender, max_contexts = max_contexts)
    ...
    def score_step(self, template_score, forward_score):
        if self.tree_scorer == gc.templateonly:
            ...
            MyLogger.print_and_log('Specified tree scoring method is not implemented. Returning product', treeEvaluator_loc, level = 2)
            return template_score * forward_score
    ...
    def work(self, i):
        ...
        # If done, stop
            if self.done.value:
                MyLogger.print_and_log('Worker {} saw done signal, terminating'.format(i), template_nn_scorer_loc)
                break
    ...
    def evaluate_tree(self, tree, context_recommender = '', context_scoring_method = '', forward_scoring_method = '', 
        ...
        if not tree['children']:
            ...
            with allow_join_result():
                ...
                score = self.score_step(reaction['template_score'] , top_result['target']['prob'])
    ...
    def spin_up_workers(self, nproc_t):
        self.running = True
        MyLogger.print_and_log('Tree evaluator spinning off {} child processes'.format(nproc_t),treeEvaluator_loc)
    ...
    def get_scored_trees(self):
        while self.waiting_for_results():
            try:
                ...
                self.scored_trees.append({'tree':tree, 'plausible':plausible, 'score':score})
            except VanillaQueue.Empty:
                pass
        self.terminate()
    def terminate(self):
        if not self.running:
            return
        self.done.value = 1
        MyLogger.print_and_log('Terminating tree evaluation process.', treeEvaluator_loc)
        
        for p in self.workers:
            if p and p.is_alive():
                p.terminate()
        MyLogger.print_and_log('All tree evaluation processes done.', treeEvaluator_loc)
        self.running = False
    ###############################################################
...
if __name__ == '__main__':
    ...
    trees = [{'is_chemical': True, 'smiles': 'CN1C2CCC1CC(C2)OC(=O)C(CO)c3ccccc3', 'ppg': 0.0, 'id': 1, 'children': [{'info': '', 'smiles': 'CN1C2CCC1CC(O)C2.O=C(O)C(CO)c1ccccc1>>CN1C2CCC1CC(C2)OC(=O)C(CO)c3ccccc3', 'is_reaction': True, 'num_examples': 19578, 'template_score': 0.017628178000450134, 'children': [{'is_chemical': True, 'smiles': 'CN1C2CCC1CC(O)C2', 'ppg': 1.0, 'id': 3, 'children': []}, {'is_chemical': True, 'smiles': 'O=C(O)C(CO)c1ccccc1', 'ppg': 1.0, 'id': 4, 'children': []}], 'id': 2, 'necessary_reagent': u''}]}]
    tree = trees[0]
    res = ev.evaluate_tree(tree, gc.nearest_neighbor, gc.probability, gc.templatebased, gc.product, is_target=True, reset = True, nproc = 16)
