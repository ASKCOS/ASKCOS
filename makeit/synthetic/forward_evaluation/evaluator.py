...
class Evaluator():
        ...
        with allow_join_result():
            ...
            if not forward_scorer:
                ...
                for i,outcomes in enumerate(all_outcomes):
                    ...
                    if not found_target:
                        MyLogger.print_and_log('Used template set did not find target in outcomes! Returning 0 as score.', evaluator_loc, level = 2)
                        evaluation_result['target']={
                                                        'smiles':target,
                                                        'template_ids':[],
                                                        'num_examples':0,
    ...
    def get_scorers(self, mincount):
        ...
        self.scorers[gc.templatebased] = load_templatebased(mincount=mincount, celery = self.celery)
