...
Input is a 256 bit fingerprint for the solvent, the reagents and catalyst
'''
...
def get_data(flow_database = None,chemicals = None, train_test_split = 0.85, write_to_file = False, read_from_file = False):
    ...
    if read_from_file:
        if all_data:
            if os.path.isfile(gc.FLOW_CONDITIONS['data_loc']):
                with open(gc.FLOW_CONDITIONS['data_loc'], 'rb') as file:
                    ...
                    MyLogger.print_and_log('Flow condition data read from {}'.format(gc.FLOW_CONDITIONS['data_loc']), flowNN_loc)
            else:
                MyLogger.print_and_log('Cannot load data from non-existent file. Exiting...', flowNN_loc, level = 3)
        else:
            if os.path.isfile(gc.FLOW_CONDITIONS_50['data_loc']):
                with open(gc.FLOW_CONDITIONS_50['data_loc'], 'rb') as file:
                    input_test = pickle.load(file)
                    input_train = pickle.load(file)
                    output_test = pickle.load(file)
                    output_train = pickle.load(file)
                    input_test_doc = pickle.load(file)
                    input_train_doc = pickle.load(file)
                    MyLogger.print_and_log('Flow condition data read from {}'.format(gc.FLOW_CONDITIONS_50['data_loc']), flowNN_loc)
            else:
                MyLogger.print_and_log('Cannot load data from non-existent file. Exiting...', flowNN_loc, level = 3)
    else:
        ...
        for doc in flow_database.find():
            ...
            fp = get_condition_input_from_instance(doc, chemicals, asone = True, use_new = True)
    ...
    if write_to_file:
        if all_data:
            with open(gc.FLOW_CONDITIONS['data_loc'], 'wb') as file:
                ...
                MyLogger.print_and_log('Flow condition data written to {}'.format(gc.FLOW_CONDITIONS['data_loc']), flowNN_loc)
        else:
            with open(gc.FLOW_CONDITIONS_50['data_loc'], 'wb') as file:
                pickle.dump(input_test, file, gc.protocol)
                pickle.dump(input_train, file, gc.protocol)
                pickle.dump(output_test, file, gc.protocol)
                pickle.dump(output_train, file, gc.protocol)
                pickle.dump(input_test_doc, file, gc.protocol)
                pickle.dump(input_train_doc, file, gc.protocol)
                MyLogger.print_and_log('Flow condition data written to {}'.format(gc.FLOW_CONDITIONS_50['data_loc']), flowNN_loc)
...
def model_to_file(model,full = True):
    ...
    if full:
        model.save(gc.FLOW_CONDITIONS['model_loc'])
    else:
        model.save(gc.FLOW_CONDITIONS_50['model_loc'])
def model_from_file(full = True):
    if full:
        return load_model(gc.FLOW_CONDITIONS['model_loc'])
    else:
        return load_model(gc.FLOW_CONDITIONS_50['model_loc'])
...
def tests():
    ...
    if FILE:
        ...
        model = set_up_model_structure(layer_nodes = layer_nodes, input = gc.fingerprint_bits)
        train_model(model, input_train, output_train, weights, 32, 25)
    ...
    #trial = np.array([get_condition_input_from_instance(instance,chemicals, use_new = True)])
    ...
    try:
        while cont:
            ...
            id = instance['RX_ID']
            inp = np.array([get_condition_input_from_instance(instance,chemicals, asone = True, use_new = True)])
            if inp == None:
                cont= True
            else:
                cont =False
        ...
        pass
    print '{} @ T:{} should be true: {}'.format(id, instance['RXD_T'], model.predict(inp))
