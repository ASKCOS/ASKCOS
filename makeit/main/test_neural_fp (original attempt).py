...
def test_embeddings_demo(model, data_fpath):
	...
	for smiles in smiles_train:
		...
		reactant = reactants.split('.')[0]
		print('first reactant: {}'.format(reactant))
		mol = Chem.MolFromSmiles(reactant)
		if not mol:
			print('  could not parse!')
			continue
		embedding = tf([molToGraph(mol)])
		print('fingerprint: {}'.format(embedding))
		print('model.layers[0].input: {}'.format(model.layers[0].input))

	print(tf)
	print('------------------DEBUG---------------------')
	theano.printing.debugprint(tf.function)
	print('----------------END DEBUG ------------------')
	...
	try:
		if input_to_bool(config['TESTING']['test_embedding']):
			...
			save_model(model, 
				...
				tstamp = tstamp)
			print('...saved model')
			print('...saving extra information...')
			print(model._predict)
			print(model._predict.function)
			theano.printing.pydotprint(model._predict.function)
			quit(1)
	except KeyError:
		pass
