...
from makeit.main.core_v2 import build_model, train_model, save_model
...
# Saving to...
now = int(time.time())
fpath = 'makeit/models/Abraham_v2_04-29-16a_CV_{}'.format(now)
os.makedirs(fpath)

# Define fixed model architecture and build
model_kwargs = {
	'padding': False,   # if batch_size > 1
	'hidden': 50,      # number of hidden nodes
	'depth': 5,        # radius for blending
	'embedding_size': 512, # fingerprint length
}
...
for fold_num in range(1, N_folds + 1):
	...
	for i in range(N_trials):
		A = np.power(10., np.random.uniform(-4., -2.))
		decay = np.random.uniform(20., 50.)
		params = {
			'lr_func': 'float(0.0007 * np.exp(- epoch / 30.))',
