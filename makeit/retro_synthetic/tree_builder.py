...
class TreeBuilder:
    ...
    def __init__(self, retroTransformer = None, pricer = None, max_branching = 20, max_depth = 3, expansion_time = 240, 
        ...
        if self.celery:
            ...
            def waiting_for_results(is_ready):         
                waiting = [expansion_queue.empty() for expansion_queue in self.expansion_queues]
        ...
        if self.celery:
            def get_ready_result():
                ...
                for i in is_ready:
                    (smiles, precursors) = self.pending_results[i].get(timeout=0.2)
                    self.pending_results[i].forget()
                    _id = self.chem_to_id[smiles]
                    yield (_id, smiles, precursors, is_ready)
                self.pending_results = [res for (i, res) in enumerate(pending_results) if i not in is_ready]
        else:
            def get_ready_result():
        ...
        if self.celery:
            def expand(smiles, chem_id, depth):
                #Chiral transformation or heuristic prioritization requires same database
    ...
    def work(self, i):
        ...
        # If done, stop
            if self.done.value:
                MyLogger.print_and_log('Worker {} saw done signal, terminating'.format(i), treebuilder_loc)
                break
            ...
            for j in range(len(self.expansion_queues))[::-1]:
                try:
                    (_id, smiles) = self.expansion_queues[j].get(timeout = 0.5) # short timeout
                    self.idle[i] = False
