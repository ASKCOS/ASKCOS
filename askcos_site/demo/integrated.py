if __name__ == '__main__':
	...
	print_and_log('###########################################################')
	time.sleep(2)
	print_and_log('IDDDFS trees:')
	trees = builder.get_trees_iddfs(max_trees = max_trees)
	print_and_log(trees)
	print_and_log('{} trees'.format(len(trees)))
	...
	def check_this_reaction(tree, this_is_the_target = False):
		...
		if rxn_smiles in plausibility_dict:
			...
			print_and_log('REACTANTS: {}'.format(reactants))
			print_and_log('PRODUCTS: {}'.format(products))
			print_and_log('necessary reagents: {}'.format(necessary_reagent))
			print_and_log('Recommended context: {}'.format([T1, t1, y1, slvt1, rgt1, cat1]))
			print_and_log('Plausible? {}'.format(plausible))
			print_and_log(outcomes)
	...
	for i in range(5):
		print_and_log('sleeping...')
		time.sleep(3)

	print_and_log('Anything completely plausible?')
	#trees = builder.get_trees_iddfs(max_trees = max_trees)
	for tree in trees:
