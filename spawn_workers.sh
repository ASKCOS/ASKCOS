...
<<<<<<< HEAD
celery -A askcos_site worker -c 6 -Q tb_worker,tb_worker_reservable -n "treebuilder_worker_pool1@$(hostname)" --maxtasksperchild 10000 --loglevel=$CELERY_LOG_LEVEL --logfile=celery_logs/%p.log &
celery -A askcos_site worker -c 6 -Q cr_worker,cr_worker_reservable -n "chiralretro_worker_pool1@$(hostname)" --maxtasksperchild 10000 --loglevel=$CELERY_LOG_LEVEL  --logfile=celery_logs/%p.log &
celery -A askcos_site worker -c 6 -Q cr_worker,cr_worker_reservable -n "chiralretro_worker_pool2@$(hostname)" --maxtasksperchild 10000 --loglevel=$CELERY_LOG_LEVEL  --logfile=celery_logs/%p.log &
=======
