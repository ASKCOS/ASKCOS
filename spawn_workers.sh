...
celery -A askcos_site worker -c 6 -Q tb_c_worker,tb_c_worker_reservable -n "tb_c_worker_pool1@$(hostname)" --maxtasksperchild 10000 --loglevel=$CELERY_LOG_LEVEL  --logfile=celery_logs/%p.log &
celery -A askcos_site worker -c 6 -Q tb_c_worker,tb_c_worker_reservable -n "tb_c_worker_pool2@$(hostname)" --maxtasksperchild 10000 --loglevel=$CELERY_LOG_LEVEL  --logfile=celery_logs/%p.log &
celery -A askcos_site worker -c 6 -Q tb_c_worker,tb_c_worker_reservable -n "tb_c_worker_pool3@$(hostname)" --maxtasksperchild 10000 --loglevel=$CELERY_LOG_LEVEL  --logfile=celery_logs/%p.log &
