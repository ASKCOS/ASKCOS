...
KERAS_BACKEND=theano THEANO_FLAGS=device=cpu celery -A askcos_site worker -c 2 -Q tb_c_worker,tb_c_worker_reservable -n "tb_c_worker_pool1@$(hostname)" --maxtasksperchild 5000 --loglevel=$CELERY_LOG_LEVEL  --logfile=celery_logs/%p.log &

# Tree builder coordinator
KERAS_BACKEND=theano THEANO_FLAGS=device=cpu celery -A askcos_site worker -c 1 -Q tb_coordinator -n "tb_coordinator@$(hostname)"  --maxtasksperchild 5 --logfile=celery_logs/%p.log &

# Tree evaluator coordinator
celery -A askcos_site worker -c 1 -Q te_coordinator -n "te_coordinator@$(hostname)"  --maxtasksperchild 5 --logfile=celery_logs/%p.log &

# Scoring coordinator
celery -A askcos_site worker -c 1 -Q sc_coordinator -n "sc_coordinator@$(hostname)"  --maxtasksperchild 100 --logfile=celery_logs/%p.log &

# Template-based forward predictor
celery -A askcos_site worker -c 1 -Q ft_worker -n "ft_worker@$(hostname)" --max-tasks-per-child 10000 --loglevel=$CELERY_LOG_LEVEL  --logfile=celery_logs/%p.log &
